...
conn.on('qr', qr =>
{
   ...
   console.log(`[ ${moment().format("HH:mm:ss")} ] L4LISA-BOT Ready scan now!`);
});
...
conn.sendMessage(id, 'Kenapa Kak' ,MessageType.text);
}
else if (text == 'sepi'){
...
conn.sendMessage(id, 'Succes Change Nama Group' ,MessageType.text, { quoted: m } );
...
conn.sendMessage(id, 'Succes Change Descripsi Group' ,MessageType.text, { quoted: m } );
...
axios.get(`http://enznoire.herokuapp.com/api/neolatest`).then((res) => {
	conn.sendMessage(id, '[WAIT] Proses...❗', MessageType.text)
...
if (text.includes('bot')) {
 var nomor = m.participant
 const options = {
       text: `apa manggil manggil tinggal ketik .help @${nomor.split("@s.whatsapp.net")[0]}, Ketik .help untuk menampilkan menu`,
       contextInfo: { mentionedJid: [nomor] }
 }
